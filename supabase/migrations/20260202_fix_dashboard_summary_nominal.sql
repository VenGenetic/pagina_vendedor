-- Fix Dashboard Summary to exclude Nominal Accounts
-- This ensures "Saldo en Cuentas" matches the visible cards in the dashboard.
...
WITH 
  account_totals AS (
    ...
    AND (is_nominal IS DISTINCT FROM true) -- Exclude nominal accounts (DEBITOS, CREDITOS)
  ),
  inventory_totals AS (
    SELECT 
